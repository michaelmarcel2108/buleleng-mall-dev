// Mendefinisikan tipe data yang akan diterima dari luar
interface BrandCardProps {
  id?: string;
  namaUmkm?: string;
  deskripsi?: string;
}

const BrandCard = ({ namaUmkm, deskripsi }: BrandCardProps) => {
  return (
    <div className="">
      <div className="w-full flex flex-col gap-1 outline outline-foreground rounded-lg p-2 md:p-4 hover:bg-gray-50 transition cursor-pointer">
        <div className="grid grid-cols-2 aspect-square gap-2 rounded-md overflow-hidden">
          <div className="w-full aspect-square bg-gray-500"></div>
          <div className="w-full aspect-square bg-gray-500"></div>
          <div className="w-full aspect-square bg-gray-500"></div>
          <div className="w-full aspect-square bg-gray-500"></div>
        </div>
        <div className="">
          {/* Teks di bawah ini sekarang dinamis dari database */}
          <p className="font-display text-xl font-medium line-clamp-1">
            {namaUmkm || "Nama Brand"}
          </p>
          <p className="text-sm text-gray-500 line-clamp-1 mb-1">
            {deskripsi || "UMKM Buleleng"}
          </p>

          <div className="flex flex-row gap-1 items-center text-yellow-500">
            <span className="flex flex-row">
              {/* Bintang 1 */}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clipRule="evenodd" />
              </svg>
              {/* Bintang 2 */}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clipRule="evenodd" />
              </svg>
              {/* Bintang 3 */}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clipRule="evenodd" />
              </svg>
              {/* Bintang 4 */}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clipRule="evenodd" />
              </svg>
              {/* Bintang 5 */}
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-4">
                <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.006 5.404.434c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.434 2.082-5.005Z" clipRule="evenodd" />
              </svg>
            </span>
            <p className="text-foreground/75 text-sm ml-1 text-black">(999)</p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default BrandCard;
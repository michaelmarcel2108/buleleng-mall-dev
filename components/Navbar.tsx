...
export default function Navbar() {
  ...
  const handleSuggestionClick = (type: "category" | "product", item: any) => {
    setIsSearchFocused(false);
    setSearchQuery(""); 
    setIsOpen(false);
          ...
          <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden z-50 flex flex-col animate-in fade-in slide-in-from-top-2 duration-200">
            {isSearching ? (
              <div className="px-4 py-4 text-xs text-gray-400 flex items-center justify-center gap-2">
                <svg className="animate-spin h-4 w-4 text-[#274a6a]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                Mencari rekomendasi...
              </div>
              ...
              >
                ...
                Cari <span className="font-bold text-[#274a6a]">"{searchQuery}"</span>
              </button>
            ) : (
              <div className="max-h-[70vh] overflow-y-auto">
                ...
                {suggestions.categories.length > 0 && (
                  <div className="border-b border-gray-100 p-2">
                    <p className="text-[10px] font-bold text-gray-400 uppercase px-2 pb-1.5 tracking-wider">Kategori Terkait</p>
                    {suggestions.categories.map((cat) => (
                      <button
                        key={`cat-${cat.id}`}
                        onMouseDown={() => handleSuggestionClick("category", cat)}
                      ...
                      >
                        {/* PERBAIKAN 1: div diubah menjadi span block */}
                        <span className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center shrink-0 block">
                          <svg className="w-4 h-4 text-[#274a6a]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" /></svg>
                        </span>
                        <span className="font-medium">Kategori <span className="text-[#274a6a]">"{cat.name}"</span></span>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {suggestions.products.length > 0 && (
                  <div className="p-2">
                    <p className="text-[10px] font-bold text-gray-400 uppercase px-2 pb-1.5 tracking-wider pt-1">Produk Buleleng Mall</p>
                    {suggestions.products.map((prod) => (
                      <button
                        key={`prod-${prod.id}`}
                        onMouseDown={() => handleSuggestionClick("product", prod)}
                      ...
                      >
                        <span className="w-10 h-10 shrink-0 bg-gray-100 rounded-md border border-gray-200 overflow-hidden relative block">
                          {prod.image_url ? (
                            ...
                            <svg className="w-5 h-5 text-gray-300 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                          )}
                        </span>
                        <span className="truncate font-medium flex-1">{prod.name}</span>
                      </button>
                    ))}
                  </div>
                )}
        ...
        <div ref={menuRef}>
          ...
          {isOpen && (
            <div className="absolute top-full right-4 md:right-16 mt-2 w-56 md:w-64 bg-white text-gray-800 rounded-xl shadow-xl border border-gray-100 overflow-hidden py-2 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-4 py-1 border-b border-gray-100 md:hidden text-[10px] font-semibold text-gray-400 uppercase tracking-wider mt-1 md:mt-0">Navigasi</div>
              ...
              <div className="px-4 py-1 text-[10px] font-semibold text-gray-400 uppercase tracking-wider mt-1 md:mt-0">Informasi</div>
              <Link href="/profile-koperasi" onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 text-sm text-gray-700 hover:text-[#274a6a] font-medium transition-colors">Profile Koperasi</Link>
              <Link href="/profile-developer" onClick={() => setIsOpen(false)} className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 text-sm text-gray-700 hover:text-[#274a6a] font-medium transition-colors">Profile Developer</Link>
            </div>
          )}
        </div>
      </div>

  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-4 md:p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gray-50/50">
        <h2 className="font-bold text-gray-800">Manajemen Banner / Slideshow</h2>
        <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-3">
          ...
          <button onClick={openAddModal} className="bg-[#274a6a] text-white px-4 py-2 text-sm rounded-lg hover:bg-[#1f3b54] font-medium whitespace-nowrap">+ Tambah Banner</button>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
              ...
              <th className="p-4 font-medium hidden md:table-cell">Link Tujuan</th>
          ...
          <tbody className="text-sm divide-y divide-gray-100">
            {filteredBanners.map((b) => (
              <tr key={b.id} className="hover:bg-gray-50/50 transition-colors">
                <td className="p-4 font-medium text-gray-800">
                  {b.image_url ? (
                    <img src={b.image_url} alt={b.title} className="w-24 h-12 object-cover rounded-lg bg-gray-100 border shadow-sm" />
                  ) : (
                    <div className="w-24 h-12 bg-gray-100 rounded-lg flex items-center justify-center text-[10px] text-gray-400 border border-dashed">No Img</div>
                  )}
                </td>
                <td className="p-4 text-gray-800 font-medium">{b.title || "-"}</td>
                <td className="p-4 text-gray-500 hidden md:table-cell">
                  {b.link_url ? (
                    <a href={b.link_url} target="_blank" rel="noreferrer" className="text-blue-500 hover:underline truncate max-w-xs inline-block">
                      {b.link_url}
                    </a>
                  ) : "-"}
                </td>
                <td className="p-4">
                  <div className="flex gap-2">
                    ...
                    <button 
                      onClick={() => setDeleteConfirm({ isOpen: true, id: b.id, title: b.title || "Banner ini" })} 
            ...
            {filteredBanners.length === 0 && <tr><td colSpan={4} className="p-8 text-center text-gray-500">Banner tidak ditemukan.</td></tr>}
          </tbody>
        </table>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSaveData} className="p-5 flex flex-col gap-4 overflow-y-auto">
              <div>
                ...
                <input type="text" value={editingItem.title || ""} onChange={(e) => setEditingItem({ ...editingItem, title: e.target.value })} placeholder="Opsional: Promo Spesial Buleleng..." className="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#274a6a]" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Link Tujuan (Saat Diklik)</label>
                <input type="url" value={editingItem.link_url || ""} onChange={(e) => setEditingItem({ ...editingItem, link_url: e.target.value })} placeholder="Opsional: https://..." className="w-full px-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#274a6a]" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Gambar Banner (Landscape)</label>
                {isEditMode && editingItem.image_url && !selectedFile && (
                  <img src={editingItem.image_url} alt="Preview" className="w-full h-32 object-cover rounded-lg border border-gray-200 mb-2 shadow-sm" />
                )}
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
                ...
                <button type="submit" disabled={isSaving} className="px-4 py-2 text-sm font-medium text-white bg-[#274a6a] hover:bg-[#1f3b54] rounded-lg">{isSaving ? "Menyimpan..." : "Simpan Banner"}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {deleteConfirm.isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center transform transition-all">
            ...
            <h3 className="font-bold text-gray-900 text-lg mb-2">Hapus Banner?</h3>
            <p className="text-gray-500 text-sm mb-6 leading-relaxed">
              Apakah Anda yakin ingin menghapus <span className="font-semibold text-gray-700">"{deleteConfirm.title}"</span>? Gambar ini tidak akan muncul lagi di halaman depan.
            </p>

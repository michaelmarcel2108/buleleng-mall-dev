...
export async function generateMetadata(
  ...
  const baseUrl =
    process.env.NEXT_PUBLIC_BASE_URL || "https://bulelengmall.com";
...
export default async function ProductDetailPage({
      ...
      <div className="min-h-screen bg-gray-50/50 flex flex-col justify-between pt-16">
        ...
        <div className="max-w-md mx-auto text-center py-12 px-4">
          ...
          <p className="mt-3 text-gray-500 text-sm md:text-base leading-relaxed">
            Waduh, produk UMKM yang kamu cari sepertinya tidak ada, sudah
            dihapus. Mari kita cari produk menarik lainnya di bawah ini!
          </p>
          <div className="mt-6">
            <Link
              href="/"
  ...
  const businessData = Array.isArray(mainProduct.businesses)
    ? mainProduct.businesses[0]
    : mainProduct.businesses;
  ...
  return (
    <main className="w-full min-h-screen bg-gray-50/50 py-8 md:py-16 px-4 md:px-8 relative overflow-hidden">
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            description:
              mainProduct.description?.replace(/<[^>]+>/g, "") ||
              `Beli ${mainProduct.name} di Buleleng Mall`,
            sku: mainProduct.id,
            brand: {
              "@type": "Brand",
      ...
      <section className="max-w-5xl mx-auto p-4 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 relative z-10 items-start">
        ...
        <div className="flex flex-col gap-4 w-full">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {categoriesList.map((cat: Partial<Category>) => (
              <div
                key={cat.id}
            ...
            {businessData?.name && (
              <Link
                ...
                className="text-xs px-4 py-1.5 font-medium rounded-full bg-blue-50 text-blue-800 hover:bg-blue-100 transition-colors border border-blue-100 shadow-sm"
              >
        ...
        <div className="flex flex-col w-full">
          <div className="flex flex-col items-start gap-3 w-full">
            ...
            <div
              className="text-3xl md:text-4xl font-bold text-foreground border-b border-gray-100 pb-4 w-full"
          ...
          <div className="flex flex-col gap-2 w-full mt-6">
            {mainProduct.shopee_url && (
              <Link
                ...
                className="w-full bg-[#EE4D2D] py-2 px-4 text-white text-center rounded-lg font-medium hover:opacity-90 transition-all text-sm shadow-sm"
              >
                Beli di Shopee
              </Link>
            )}
            {productData.tokopedia_url && (
              <Link
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full bg-[#00AA5B] py-2 px-4 text-white text-center rounded-lg font-medium hover:opacity-90 transition-all text-sm shadow-sm"
              >
                Beli di Tokopedia
              </Link>
            )}
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-50 bg-green-600 text-white p-4 rounded-full shadow-[0_4px_14px_0_rgba(37,211,102,0.39)] hover:bg-green-400 hover:-translate-y-1 hover:scale-110 transition-all duration-300 flex items-center justify-center"
      >
        <svg
